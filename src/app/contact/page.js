...
const page = async () => {
  const { description } = await getContentContact();

  return (
    <section
      id="maps"
      className="padding_page h-[calc(80vh-100px)] flex flex-col justify-center items-center sm:flex-row gap-[5vw] bg-primary !py-0"
    >
      ...
      <div className="mt-[4vh] sm:w-[30vw] order-[-1] sm:order-[2] padding_page sm:px-0">
        ...
        <p className="w-[95%] md:w-[36ch]">{description}</p>
      </div>
    </section>
  );
};
